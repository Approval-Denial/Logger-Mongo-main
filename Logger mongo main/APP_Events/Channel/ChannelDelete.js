...
module.exports = async(channel) => {
 ...
 let channellog =  channel.guild.channels.cache.get(DataLog.channelCreateLog)
 if(channellog) await channellog.send({embeds: [embed.setDescription(`**__Kanal Silindi__**`).addField("Silen kişi",`Adı: ${member}\n ID: \`${member.id}\``,true).addField("Kanal Bilgisi",`Kanal Adı: ${channel.name}\nID: \`${channel.id}\``,true).addField("Tarih",`${dayjs(new Date()).tz("Europe/Istanbul").format("DD MMMM YYYY HH:mm")}`)]})
   //#region  Sms Api
